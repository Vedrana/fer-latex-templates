% Upute za LaTeX predložak ZR-a i Seminara, verzija: 1.1
\documentclass[zavrsni, lmodern, utf8]{fer}
\usepackage{booktabs}
\usepackage{enumitem}
\usepackage{multicol}
\usepackage{textcomp}
...
\begin{itemize}
	...
	\item \texttt{english} -- pisanje rada na englskom jeziku.
\end{itemize}
...
Osnovna struktura \texttt{tex} datoteke za Završni rad mora biti:
\begin{verbatim}
...
Ukoliko koristite predložak za Seminar, umjesto naredbe \verb|\thesisnumber|
morate navesti naredbu \verb|\voditelj| te kao argument navedite ime i prezime
vašeg mentora ili mentorice, odnosno voditelja vašeg rada.
...
O stilu i strukturi dokumenta pogledajte \ref{ch:stil-i-struktura} poglavlje, a
za upoznavanje s \LaTeX-om te nekim njegovim posebnostima i receptima pogledajte
poglavlje \ref{ch:latex-upute}.

Napravljeni predložak sadrži rješenje za dodavanje sažetka rada i zahvale te
naredbu \texttt{engl}. Za navedeno pogledajte odjeljke \ref{sec:sazetak},
\ref{sec:zahvala} i \ref{sec:engl}.

U sam predložak su uključeni paketi \texttt{graphicx} za dodavanje slika,
\texttt{amssymb} i \texttt{amsmath} za matematičke simbole, \texttt{url}
za navođenje URL-ova, \texttt{enumitem} za proširene mogućnosti listi i
nabrajanja te \texttt{natbib} za lakši rad s literaturom.

\chapter{Stil i struktura}
\label{ch:stil-i-struktura}
Stil (format, font, prored, margine i sl.) određen je predloškom te se nije
potrebno brinuti o njemu. U nastavku će detaljnije biti opisana struktura
Završnog rada preuzeta iz uputa na stranici
\url{http://www.fer.hr/predmet/zavrad}. Sve navedeno za Završni rad vrijedi i za
Diplomski.
...
\section{Posebnosti stila za Seminar}
Za razliku od Završnog i Diplomskog rada, Seminar ne predviđa mjesto za zahvalu i
izvornik rada. Uz to, sažetak kod seminara je standardno poglavlje (ne koriste se
posebne naredbe kao kod Završnog i Diplomskog rada) koje dolazi nakon literature.
...
Među razvojnim okruženjima mogu se istaknuti \emph{Texmaker}, \emph{\TeX{}works} i
\emph{Eclipse} s \emph{Texlipse} dodatkom. Navedena razvojna okruženja su
besplatna, dostupna za sve poznatije operacijske sustave te podržavaju UTF-8
kodnu stranicu.
\begin{itemize}
...
\TeX{}works je jednostavan po pitanju podešavanja jer ima mali broj opcija, no
bitno je napomenuti da mu je standardni prevoditelj \texttt{pdftex} te je to
potrebno promijeniti u \texttt{pdflatex}. Kod \TeX{}worksa mnogima će se svidjeti
ugrađeni preglednik koji osvježava prikaz nakon prevođenja dokumenata.
...
\emph{Builder Settings} te tu podesiti putanje do \TeX{} alata. Kodna stranica
...
\texttt{documentclass} kao parametar prima definirani stil (predložak je
definiran u datoteci \texttt{fer.cls}; ostali često korišteni stilovi su
\emph{article} te \emph{report} na kojem se temelji stil razvijenog predloška).
...
\begin{enumerate}%
  ...
  \item druga stavka.%
\end{enumerate}}
  \item[Nabrajanje s proizvoljnim brojačem] (zahtjeva \texttt{enumitem} paket) -- kod:
\begin{verbatim}
...
list papira mora koristiti \texttt{longtable} paket. Upoznavanje s paketom je
najbolje napraviti u primjerima koje možete naći na
...
tablice prekidaju se sekvencom ``\verb|\\|.'' Naredba \verb|\hline| odvajaju retke horizontalnim
linijama. Preporuka je uključiti \texttt{booktabs} paket te umjesto \verb|\hline| naredbe koristiti:
\verb|\toprule|, \verb|\midrule| i \verb|\bottomrule| (redom, ispod naslova, nakon zaglavlja, na kraju tablice).
Same naredbe razlikuju se u debljini linije koju stvaraju i količinama
praznine iznad i ispod linije. Preporuka je raditi tablice bez vertikalnih
linija \citep{simon2005booktabs}.
\end{itemize}
...
(npr., ako imate stupac ``Masa'', umjesto da za podatke navodite npr., ``1 kg'',
jedinicu stavite u naslov -- ``Masa (kg)''),
  \item uvijek stavljajte broj ispred decimalne točke; znači ``0.1,'' a ne ``.1,''
...
Ako sliku trebate staviti unutar teksta, uključite paket \texttt{wrapfig}, a
za prikaz više slika u nizu koristite paket \texttt{subfig}. Za primjere korištenja
...
Za podjelu sadržaja na stupce koristite \texttt{multicol} paket. Primjer
korištenja:\footnote{Korišteni citati pripadaju Donaldu E.~Knuthu, tvorcu \TeX-a}
\begin{verbatim}
\begin{multicols}{2}
...
\texttt{minipage} paket, primjerice:
\begin{verbatim}
...
Za korištenje tablica iskoristite također \texttt{minipage} paket sa
...
referenciranja preko naredbe \verb|\bibliographystyle|. Uz predložak, preveden
je stil \texttt{natbib} paketa \texttt{plainnat.bst} i spremljen kao \texttt{fer.bst} 
(uključuje se naredbom \verb|\bibliographystyle{fer}|).
...
Pretpostavljeni način citiranja je ``(autor, godina).'' Ako želite promijeniti stil
citiranja u ``[indeks]'' (npr.\ [1]) tada prilikom učitavanja stila dodajte parametar
``numeric,'' primjerice \verb|\documentclass[zavrsni, numeric]{fer}|.
...
\section{Dodavanje zahvale}
\label{sec:zahvala}
Zahvala se dodaje naredbom \verb|\zahvala| na mjestu gdje želite stranicu s
zahvalom (npr.\ prije tablice sadržaja -- iznad \verb|\tableofcontents|
naredbe). Primjer korištenja:
\begin{verbatim}
\zahvala{Svima koji su pomogli u izradi predloška i uputa.}
\end{verbatim}
