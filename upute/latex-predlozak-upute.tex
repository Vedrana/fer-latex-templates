% Upute za LaTeX predložak ZR-a i Seminara, verzija: 0.3beta
...
problemima vezanim uz formatiranje teksta Završnog i Diplomskog rada te Seminara
koji su obrađeni jednostavnim primjerima.

\LaTeX{} je sustav za visokokvalitetno slovoslagarstvo. Omogućava jednostavnu, a
...
znanstvenik, Donald E.~Knuth. Sam \LaTeX{} je također napisan od utjecajne
znanstvene ličnosti, Lesliea Lamporta.
...
Primjerice, izgled dokumenta, čiji je sadržaj napisan te struktura složena, izgled
mijenja jednostavnom izmjenom predloška.\footnote{Predložak je skup definicija stila.}

Bitno je za napomenuti da \LaTeX{} omogućava jednostavnu izradu lijepih dokumenata
po predefiniranim stilovima. Veći stilski zahvati znaju biti izuzetno teški (čak i
izrada ovog predloška) i zahtjevaju bolje poznavanje \LaTeX-a ili čak samog
...
poput \emph{Microsoft Worda}, \emph{OpenOffice Writera}. U svrhu poticanja
...
publikaciju dokumenata. Slijedeća bitna prednost je činjenica da se \TeX{}
...
olakšava pisanje velikim dokumenata, primjerice, korištenjem \LaTeX-ove
...
Cilj ovih uputa je omogućiti korisniku što bezbolniju izradu Diplomskog rada, Završnog
...
u vašem radu nemate potrebu za korištenjem tablica ili dodatnih paketa, nemojte se zamarati
...
Radi jednostavnosti, konkretnosti i sličnosti među razvijenim predlošcima za
upute će se uvijek referencirati na Završni radi datoteku \texttt{zavrsni.cls}
...
Stil Završnog rada (format, font, prored, margine i sl.) je određen predloškom te
...
\end{enumerate}
Završni rad mora imati sažetak od približno stotinjak riječi. Prilikom navođenja
...
\texttt{pslatex} za \texttt{ps}) te alate za konverziju između formata (npr.\
\texttt{ps2pdf}).
...
rezultira sa ``tri   razmaka.'' Dodatni razmak možete postići naredbom
\verb|\hspace{}|.

\subsection{Naredbe i okoline}
Izgled, stil i struktura \LaTeX{} dokumenta se definiraju naredbama i
okolinama. Naredbe su jednostavni elementi \LaTeX--a koji primaju argumente te
se transformiraju u oblikovani tekst. Naredbe počinju s unazadnom kosom crtom
``\verb|\|'' koju slijedi naziv naredbe te njeni argumenti. Primjeri poziva
naredbe bez argumenta, sa jednim argumentom te sa dva argumenta:
\begin{verbatim}
\naredba
\naredba{a}
\naredba{a}{b}
...
Početak djelovanja okoline se označava naredbom \verb|\begin{}|, a kraj
...
\end{okolina}
\end{verbatim}
Dodatni argumenti i parametri okoline se navode uz naredbu \verb|\begin{}|.
...
Radi poštivanja pravila tipografije, \LaTeX{} ima posebnosti koje morate
poštivati da bi vaši dokumenti izgledali što ljepše.

Osnovni problem na koji će te nailaziti proizlazi iz \LaTeX-ovih pokušaja
...
Po tipografskim pravilima nakon kraja rečenice (tj.\ točke) trebalo bi doći malo
više od standardnog razmaka. Radi toga, \LaTeX{} pretpostavlja da je svaka točka
...
uz uvijet da se na tom razmaku linija teksta ne smije prelomiti, tj.\ ako napišete
``\verb|prof.~dr.~sc.~Donald~E.~Knuth|'' \LaTeX{} vam garantira da će se cijelo ime sa punom
...
Ako pogledate dokumente napravljene \LaTeX-om, možete primjetiti različite vrste navodnika
...
zatvara (prisjetite se samo problema sa krajem rečenice), stoga morate koristiti posebne sekvence
...
stoga, dane upute su samo prijedlog. Ako trebate ``minus'' ispred broja uvijek uđite u mod za matematiku.
...
Faculty of Electrical Engineering and Computing. The manual consists of part which
explain use of templates and part which explain \LaTeX{} in general. The \LaTeX{}
part is made with to be sufficient as an introduction to solving the text formating
problemes but still relativly short.
